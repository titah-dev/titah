...
**Scope note.** All four of #1–#4 came from one change of exposure and were
originally one change. #3 (the atomic flush) and #4 (the pruner's per-tool policy)
shipped separately once it was clear they were settled while this half was not —
three review rounds found eleven further defects here, including a regression
against `main`. Their record is
[`2026-08-12-storage-and-pruner-design.md`](./2026-08-12-storage-and-pruner-design.md);
the problem statement below keeps all four so the cycle still reads as one story.
Issue #5 (intent state) is a design pass with no code:
